# Doom Loop Detection Specification

## Purpose

Detect and self-terminate agents stuck repeating the same failing tool call: when the same tool call fails with the same error three consecutive times (exact match on `tool_name` plus the first 200 characters of the error text), any agent self-terminates with a descriptive stop message. Any successful tool call resets the failure streak. This capability is the universal runtime backstop that replaced the removed step-count caps.
