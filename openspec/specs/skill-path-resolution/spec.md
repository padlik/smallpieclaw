# skill-path-resolution Specification

## Purpose
When the harness returns `SKILL.md` content via `file_read`, relative path references authored per the Agent Skills convention (`./scripts/run.sh`, `assets/.env`, etc.) are substituted with absolute paths rooted at the skill directory before the content reaches the agent. This removes reliance on the LLM remembering a prompt instruction (e.g. `cd <skill_dir> &&`) during multi-step skill execution.
