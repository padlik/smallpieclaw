...
Rule: Agent-scoped secrets live in a single TOML file, referenced by key name.

#### Scenario: Vault file exists and contains secrets
- **GIVEN** the `[vault]` section has `type = "file"`
- **AND** a TOML file exists at `~/.local/share/<agent_name>/secrets.toml`
...
- **AND** the default `~/.local/share/<agent_name>/secrets.toml` is ignored
