# SQLite Log Store Specification

## Purpose

Define the storage mechanics of the structured log primary sink: a SQLite WAL database (`agent_logs.sqlite`) with wide-column rows, single-writer queue ingestion, time-based retention, and graceful degradation to prose-only operation. Query behavior over the store is defined by the `runtime-log-introspection` capability; the structured-stream contract it serves is defined by `structured-event-logging`.
