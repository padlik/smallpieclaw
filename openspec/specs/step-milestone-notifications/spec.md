# Step Milestone Notifications Specification

## Purpose

Keep the operator aware of long-running main-agent work: at each positive multiple of `step_notify_interval` completed steps, the main agent sends a fire-and-forget Telegram milestone notification that does not block the react loop. `step_notify_interval = 0` disables notifications. Scheduled, plan-step, and on-demand sub-agents never send milestone notifications.
