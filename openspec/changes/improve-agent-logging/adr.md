...
- **ADR-0003 — Use TOML for agent-scoped vault files** (Accepted, supersedes ADR-0002). Relevant here only because secret redaction sources its known values from the agent-scoped vault this ADR governs; the logging change is coherent with it and does not revisit it.

Superseded (historical context only, not in force): ADR-0001 (file-backed provider secrets), ADR-0002 (agent-scoped vault).

## New Durable ADRs Created

- **ADR-0004 — Structured-primary agent logging with contextvars identity** (`adr/0004-structured-primary-agent-logging.md`). Records the durable commitment to a structured-primary dual-sink logging architecture with `contextvars`-based record identity and a closed event taxonomy, and the deferral of a SQLite store. Does not supersede any prior ADR.
