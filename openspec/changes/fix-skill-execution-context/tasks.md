...
- [ ] Task 1: Add `skill_registry` attribute to `BuiltinExecutor`
...
- [ ] Task 2: Implement `_expand_skill_paths(content, skill_dir)` in `builtin_tools/files.py`
...
- [ ] Task 3: Wire `_expand_skill_paths` call into `_run_file_read`
  - Files: `builtin_tools/files.py`
...
- [ ] Task 4: Wire `skill_registry` onto executor in `main.py`
  - Files: `main.py`
...
- [ ] Task 5: Add tests for `_run_file_read` SKILL.md interception
...
- [ ] Task 6: Remove `cd <skill_dir> &&` instruction from `prompt_builder.py`
