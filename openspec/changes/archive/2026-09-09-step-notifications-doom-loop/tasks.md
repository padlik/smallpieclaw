...
- [x] T1.1 Define module-level sentinel: `_EFFECTIVELY_UNLIMITED_STEPS = 10_000_000` (retain existing value if already present; do not rename or delete).
- [x] T1.2 Replace the nested `while True: / while state.step < state.max_steps:` structure with a single `while True:` loop. Update `_LoopState.__init__` to always set `max_steps = _EFFECTIVELY_UNLIMITED_STEPS` regardless of agent type.
- [x] T1.3 Delete `_handle_step_limit_reached()` (lines 1594–1615 in the current codebase) and its call site entirely.
- [x] T1.4 Delete the existing `"⚠️ Agent reached maximum steps. Operation cancelled."` post-loop fallback — it is dead code after gate removal. The post-loop return after `break` MUST be `"⚠️ Task stopped by operator."` (the operator-cancel message, since `break` is only set by the operator-cancel `should_continue=False` path).
- [x] T1.5 Verify at apply time: before removing the between-loop `operator_cancelled` check, confirm that every code path in `_run_single_step` that sets `state.operator_cancelled = True` also yields `result.early_return` (non-None) or `result.should_continue = False`. If any cancel path relies solely on the state flag, move the check inside the single loop rather than deleting it.
...
- [x] T2.1 Define module-level constant: `_DOOM_LOOP_LIMIT = 3` (mirrors `_JSON_FAIL_LIMIT = 3`).
- [x] T2.2 Add `_last_tool_fail_key: str = ""`, `_tool_fail_repeat: int = 0`, and `_last_notified_step: int = -1` to `_LoopState`. (`_last_notified_step` is used by T4's milestone double-fire guard.)
- [x] T2.3 Place the doom-loop check **inside `_dispatch_action()`**, in the tool-dispatch branch after the tool outcome is obtained — this is the merge point covering both `vision_query` and normal-tool paths. The `outcome` dict is only in scope here; `_run_single_step()` never sees `success`/`error`.
...
- [x] T2.4 **Checkpoint non-persistence (stated decision):** `_last_tool_fail_key` and `_tool_fail_repeat` are NOT persisted in the checkpoint dict. On resume, they default to `""`/`0` (fresh streak). This is intentional — a resumed run should not carry over a pre-resume failure streak.
...
- [x] T3.1 `react_loop.py`: remove or simplify step-counter log lines referencing `max_steps`
- [x] T3.2 `telegram_interface.py:1049`: update step display in RunPanel
- [x] T3.3 `telegram_commands.py:422`, `:429`, `:457`: update `/status` step display
- [x] T3.4 `sub_agent_supervisor.py:457`: update sub-agent step display
...
- [x] T4.1 Add to `ReactContext`:
   ```python
   step_notify_interval: int = 0
   milestone_notify_fn: Optional[Callable[[int], None]] = None
   ```

- [x] T4.2 Add `RunPanel.milestone_notify(step: int)` method that sends a non-blocking Telegram message.

- [x] T4.3 In `agent_controller.py`, wire `step_notify_interval` from config and pass `milestone_notify_fn = run_panel.milestone_notify` when building `ReactContext` for the main agent. Sub-agent `ReactContext` instances get `milestone_notify_fn = None`.

- [x] T4.4 In `react_loop.py`, after each completed step:
   ```python
...
- [x] T5.1 Remove `_handle_extend_progress` in `telegram_interface.py`
- [x] T5.2 Remove `_send_extend_prompt` in `telegram_interface.py`
- [x] T5.3 Remove `__EXTEND__:` signal handling in `telegram_interface.py`
- [x] T5.4 Remove extend-button callback handlers in `telegram_callbacks.py` (preserve all other dispatch entries and handlers)
- [x] T5.5 Update `vulture_whitelist.py` (`request_extension`, `EXTEND_PREFIX`) and update the `confirmation.py` docstring (lines 190–191) to reflect the step-extension flow is now dormant
...
- [x] T6.1 Add `step_notify_interval: int = 30` to the appropriate config dataclass.
- [x] T6.2 Mark `max_iterations`, `scheduled_max_iterations`, and `plan_max_iterations` as silently ignored — read and discarded at load time, no deprecation warning emitted. Retain the fields for backward compat so existing config files do not cause startup errors.
...
- [x] T7.1 Remove from descriptor JSON schema in `descriptors.py`
- [x] T7.2 Remove from validation schema in `schemas.py`
- [x] T7.3 Remove plumbing in `schedule.py`
...
- [x] T8.1 Remove `max_iterations` plumbing from `spawn_agent` in `agents.py`.
- [x] T8.2 **Open question (verify at apply time):** Check whether `spawn_agent` descriptor in `descriptors.py` exposes `max_iterations` to the LLM. If yes, remove it. — Verified at apply: the descriptor did NOT expose it (internal plumbing only); the `agents.py` plumbing is removed.
...
- [x] T9.1 In `scheduler.py`, remove the `max_iterations` override plumbed from the schedule tool call through to sub-agent construction. Scheduled agents inherit universal unlimited runtime.
- [x] T9.2 Update `scheduler.toml` documentation (comments or README section) to remove references to `max_iterations` as a job-level parameter.
...
- [x] T10.1 `make check` (ruff + vulture + pytest) passes clean.
- [x] T10.2 Doom-loop unit test: 3× identical `(tool_name, error)` → loop aborts with doom-loop message; success resets `_tool_fail_repeat` to `0` and `_last_tool_fail_key` to `""`; 3× different errors do not trigger doom. Include a scheduled-agent path: confirm doom-loop fires identically when the run originates from the scheduler (same `react_loop` code path, so structural coverage is sufficient).
- [x] T10.3 Milestone notification unit test: `milestone_notify_fn` called at step 30/60/90; NOT called at step 0; NOT called when `step_notify_interval = 0`.
- [x] T10.4 Operator-cancel regression: confirm that operator cancellation still terminates the loop correctly after loop collapse (either via existing test or a targeted inspection check).
- [x] T10.5 **Max-step test cleanup (S6):** Identify tests that assert the loop halts at `max_iterations` or produce the `"maximum steps"` return string — these are behavior-removal cases, not regressions to preserve. Rewrite or delete them as part of this task; do not make the code pass them artificially. Search: `grep -rn "maximum steps\|max_iterations" tests/`.
- [x] T10.6 Regression: existing tests for confirm flow, headless confirm, `json_fail_streak`, and scheduled jobs pass unmodified.
