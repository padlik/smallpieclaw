...
- **AND** the vault path defaults to `~/.local/share/piclaw/secrets.json`
...
- **AND** the vault path defaults to `~/.local/share/mybot/secrets.json`
...
- **AND** the vault path still defaults to `~/.local/share/piclaw/secrets.json`
...
- **AND** the environment variable `SPC_VAULT_FILE` is set to `/run/secrets/mybot.json`
- **WHEN** the application starts
- **THEN** the vault is loaded from `/run/secrets/mybot.json`
- **AND** the default `~/.local/share/mybot/secrets.json` is ignored
