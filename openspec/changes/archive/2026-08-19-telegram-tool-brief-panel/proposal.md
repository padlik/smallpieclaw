...
- Mark MCP tools with `[MCP:server_name]` in the brief, using `mcp_manager.server_name_for_tool()` to look up the server name at TOOL_START time.
...
- **`mcp_client.py`** — read-only usage (`MCPManager.server_name_for_tool()` already exposes the server name for a tool).
